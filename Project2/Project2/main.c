...
char* returnword(char* line, int n);
...
int CheckUserName(char* username)
{
	if (strlen(username) <= 20)
	{
		...
		{
			if ('A' <= username[i] && username[i] <= 'Z') // checking if any char is between A-Z
				capital = 1;
			if ('a' <= username[i] && username[i] <= 'z') // checking if any char is between a-z
				small = 1;
			if ('0' <= username[i] && username[i] <= '9') // checking if any char is between 0-9
				digit = 1;
		}
		return (small || capital) || digit;
	}
	else
		return 0;
}
...
int PrintClientMenu(char* username) // Client menu
{
	...
	while (choose != 1 && choose != 2 && choose != 3 && choose != 4)
	{
		...
		{
			char check[256] = "-1";//
			while (atoi(check) != 1 && atoi(check) != 2)//
			{
				printf("If you will log out your cart will be deleted, are you sure? (1- Yes, 2- No): ");//  // Added 1.12.21
				gets(check);//
				if (atoi(check) != 1 && atoi(check) != 2)//
...
int CheckIfItemExistsInShoppingCart(char** List, int* Current_Size_list, char Product_Details[])
{
	for (int i = 0; i < *Current_Size_list; i++)
	{
		if (atoi(returnword(List[i],1)) == atoi(returnword(Product_Details, 1)))
			return 1;
	}
	return 0;
}
...
void Print_Shopping_Cart_Items(char** List, int* Current_Size_list) // Printing the shopping cart
{
	...
	{
		printf("%s\n", List[i]);
		count += atoi(returnword(List[i], 3)) * atoi(returnword(List[i], 4)); // Added 1.12.21
	}
...
void Delete_Shopping_Cart(char** List, int* Current_Size_list) // Deleting the shopping cart
{
	...
	for (i = 0; i < *Current_Size_list; i++)
	{
		...
		sprintf(NewQuantity, "%d", (atoi(returnword(GetProductBySerial(returnword(ProductDetails, 1)), 4))) + atoi(returnword(ProductDetails, 4)) - atoi(NewQuantity));
		FILE* pl;
		int num1;
		...
		{
			fgets(line, 150, pl);
			if (strcmp(ProductDetails, returnword(line, 2)) != 0)
			{
		...
		free(newpro);
		strcpy(line2, returnword(ProductDetails, 0));
		strcat(line2, " ");
		strcat(line2, returnword(ProductDetails, 1));
		strcat(line2, " ");
		strcat(line2, returnword(ProductDetails, 2));
		strcat(line2, " ");
		strcat(line2, returnword(ProductDetails, 3));
		strcat(line2, " ");
...
char** Shopping_Cart(char** List, int* Number_Of_Products, int* Number_Of_Total_Carts, int* Updadated_Serial, char* User)
{
	...
	while (atoi(Choice) != 5)
	{
		...
		if (atoi(Choice) == 2)//
		{
			...
			while (atoi(check) != 1 && atoi(check) != 2)//
			{
				printf("Are you sure you want to delete your shopping cart? (1- Yes, 2- No): ");//    // Added 1.12.21
				gets(check);//
				if (atoi(check) != 1 && atoi(check) != 2)//
					printf("Wrong choice, please choose again\n");//
			}
		...
		case EDIT:
			...
			while ((atoi(NewQuantity) == 0 && strcmp(NewQuantity, "0") != 0) || atoi(NewQuantity) < 0 || (atoi(returnword(GetProductBySerial(returnword(ProductDetails, 1)), 4)) + atoi(returnword(ProductDetails, 4))) < atoi(NewQuantity))// added 1.12.21
			{
				...
				gets(NewQuantity);
				if ((atoi(NewQuantity) == 0 && strcmp(NewQuantity, "0") != 0 ) || atoi(NewQuantity) < 0 || (atoi(returnword(GetProductBySerial(returnword(ProductDetails, 1)), 4)) + atoi(returnword(ProductDetails,4))) < atoi(NewQuantity))// added 1.12.21
					printf("Please choose numbers only!\nPlease choose a number between 0 to the amount in the inventory\n");// added 1.12.21
			}
...
char* returnword(char* line, int n)//מחזיר את השם
{
	char* temp = (char*)malloc(MAXSTRING);
	if (temp == NULL)
		return 1;
	int count = 0, i = 0, j, count2 = 0;
	while (count != n)
...
void filterItemsByHighPrices(void)
{
	int countItems = countLines(), i = 0, num = 10;
	FILE* read = fopen("categories.txt", "r");
	char singleline[150];
	...
	while (run)
	{
		...
		case 3:
			...
			while (atoi(Product_Details) != 0)
			{
				...
				while (atoi(Quantity) <= 0 || atoi(returnword(GetProductBySerial(Product_Details),4)) < atoi(Quantity)) // This while addad 1.12.21
				{
					printf("Please choose the amount you want to get (has to be between 0 to the amount in the inventory):\n");
					gets(Quantity);
					count1++;
					if (count1 == 3)
					{
						printf("Too many Tries...\nAn error has been occurrend\n");
			...
			if (CheckIfItemExistsInShoppingCart(List, Current_Size_list, Product_Details) == 0) // This addad 1.12.21
			{
				List = Add_New_Item_To_Shopping_Cart(List, Current_Size_list, Product_Details);// This addad 1.12.21
				List = Change_Quantity_Of_A_Product(List, Current_Size_list, Product_Details, Quantity);// This addad 1.12.21
			}
			else // This addad 1.12.21
			{
				for (int i = 0; i < *Current_Size_list; i++)// This addad 1.12.21
				{
					if (atoi(List[i], 1) == atoi(Product_Details, 1))
						sprintf(Quantity, "%d", atoi(Quantity) + atoi(returnword(List[i], 4)));;// This addad 1.12.21
				}
				List = Change_Quantity_Of_A_Product(List, Current_Size_list, Product_Details, Quantity);// This addad 1.12.21
			}
		...
		case 4:
			...
			char choice[256] = "-1";
			while (atoi(choice) != 1 && atoi(choice) != 2 && atoi(choice) != 3)
			{
				printf("1. Low - High\n2. High - Low\n3. Return\n");
				gets(choice);
				switch (atoi(choice))
	...
	while (run)
	{
		...
		if (MorC == 'C')
		{
			while (run2)
			{
				...
				case CART:
					shoppingcart = Shopping_Cart(shoppingcart, Number_Of_Products, Number_Of_Total_Carts, UpdatedSerial, username);
					break;
